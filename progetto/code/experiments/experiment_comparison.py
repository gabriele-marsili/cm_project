"""IRLS vs SGPTL on a moderate problem (H=50, M=200, lam=0.1): iterations and CPU time to reach a grid of accuracy targets, plus support-recovery"""
...
from _plot_style import (apply_style, style_axes, COLOR_IRLS, COLOR_DSM, COLOR_FCUR, SIZE_DOUBLE)
apply_style()
...
def run() -> None:
    ...
    H, M = 50, 200
    X, y, _, f_star, w_star = make_lasso_problem(n=H, m=M, sparsity=0.1, noise_std=NOISE, lam=LAMBDA, random_state=SEED)
    ...
    res_irls = irls(X, y, LAMBDA, eps_thr=1e-8, eps_stop=1e-12, k_max=300, solver="cholesky", w0=w_ols, f_star=f_star)
    # sgptl with ols (to make it comparable to the irls algorithm).
    res_dsm = deflected_subgradient(X, y, LAMBDA, w0=w_ols, i_max=30000, beta=1.0, delta0=0.1 * f_w0, rho=0.7, f_star=f_star)
    ...
    rows = []
    print(f"\n{'eps':>6}  {'IRLS iter':>10}  {'IRLS time':>10}  {'SGPTL iter':>11}  {'SGPTL time':>11}")
    for eps in eps_grid:
    ...
    irls_floor = 1e-7
    irls_cut = (int(np.argmax(irls_gaps <= irls_floor)) + 1 if np.any(irls_gaps <= irls_floor) else len(irls_gaps))
    irls_gaps = irls_gaps[:irls_cut]

    ax = axes[0]
    irls_iters = np.arange(1, len(irls_gaps) + 1)
    dsm_iters = np.arange(1, len(dsm_gaps) + 1)
    ax.loglog(irls_iters, irls_gaps, color=COLOR_IRLS, marker="o", markersize=4.5, linewidth=2.0, label="IRLS")
    ax.loglog(dsm_iters, dsm_gaps, color=COLOR_DSM, linewidth=2.0, label=r"SGPTL  $(\bar{f}^{i} - f^{*})/|f^{*}|$")
    ax.scatter([1], [irls_gaps[0]], s=80, marker="*", color="black", zorder=6, label="OLS warm start (shared)")
    ...
    smallest = min(irls_t[1] if len(irls_t) > 1 else 1e-5, dsm_t[1]  if len(dsm_t)  > 1 else 1e-5)
    t_start = smallest / 2.0
    irls_t_plot = irls_t.copy(); irls_t_plot[0] = t_start
    dsm_t_plot = dsm_t.copy();  dsm_t_plot[0]  = t_start

    ax.loglog(irls_t_plot, irls_gaps, color=COLOR_IRLS, marker="o", markersize=4.5, linewidth=2.0, label="IRLS")
    ax.loglog(dsm_t_plot, dsm_gaps, color=COLOR_DSM, linewidth=2.0, label=r"SGPTL  $(\bar{f}^{i} - f^{*})/|f^{*}|$")
    ax.scatter([t_start], [irls_gaps[0]], s=80, marker="*", color="black", zorder=6, label="OLS warm start (shared)")
    ...
    fig.suptitle(rf"IRLS vs SGPTL  ($H={H}$, $M={M}$, $\lambda_{{\mathrm{{LASSO}}}}={LAMBDA}$)", y=1.00, fontsize=15)
    fig.tight_layout()
    ...
    for tol in (1e-6, 1e-3):
        for tag, w_h in (("IRLS", w_irls), ("SGPTL", w_dsm), ("true", w_star)):
            ...
            print(f"  {tol:>6.0e}  {tag:>6}  {m['sparsity']:>8.0%}  {m['precision']:>9.3f}  {m['recall']:>7.3f}  {m['f1']:>5.3f}")
            rows_supp.append([tol, tag, m['sparsity'], m['precision'], m['recall'], m['f1']])

...
def test_grad_smooth_finite_difference(rng, small_problem):
    ...
    eps = 1e-6
    for _ in range(5):
        # w with all components bounded away from zero
        w = rng.uniform(0.5, 1.5, size=n) * rng.choice([-1.0, 1.0], size=n)
        g_smooth = grad_smooth(p.X, p.y, w)
        # at smooth points the full gradient is g_smooth + lam*sign(w)
...
def test_grad_smooth_zero_when_residual_is_zero():
    """if Xw = y, the smooth gradient is exactly zero"""
...
def test_subgradient_minimum_norm_at_zero():
    """min-norm subgradient at w=0

    The min-norm element is grad_smooth - clip(grad_smooth, -lam, lam). Our code
    returns grad_smooth + lam*sign(0) = grad_smooth, which is min-norm when
    |grad_smooth_i| <= lam (there s_i = 0 minimises (g_i + lam*s_i)^2 over
    s_i in [-1,1]) and stays in the subdifferential.
    """
    rng = np.random.default_rng(0)
    n = 8
    ...
    # valid subgradient -> g - grad_smooth must lie in [-lam,lam]^n
    s = (g - grad_smooth(X, y, w)) / lam
    assert np.all(np.abs(s) <= 1.0 + 1e-12)
    # in this regime the min-norm choice is s = 0, so g = grad_smooth

...
\begin{proof}[Proof sketch]
    ...
    $$\mathbf{X}^T(\mathbf{Xw}^* - \mathbf{y}) + 2\lambda_{\text{IRLS}}\mathbf{W}_*^T\mathbf{W}_*\mathbf{w}^* = 0$$
    Since $(\mathbf{W}_*)_{ii} = |w^*_i|^{-1/2}$, we have
    $(\mathbf{W}_*^T\mathbf{W}_*\mathbf{w}^*)_i = |w^*_i|^{-1} w^*_i = \mathrm{sign}(w^*_i)$,
    so this reduces to:
    $$\mathbf{X}^T(\mathbf{Xw}^* - \mathbf{y}) + 2\lambda_{\text{IRLS}}\,\mathrm{sign}(\mathbf{w}^*) = 0$$
    Recalling our mathematically derived relation $\lambda_{\text{IRLS}} = \frac{1}{2}\lambda_{\text{LASSO}}$ from Equation~\eqref{eq:lambda_relation}, we substitute to obtain:
    $$\mathbf{X}^T(\mathbf{Xw}^* - \mathbf{y}) + \lambda_{\text{LASSO}}\,\mathrm{sign}(\mathbf{w}^*) = 0$$

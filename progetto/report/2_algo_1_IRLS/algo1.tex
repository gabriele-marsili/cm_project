...
The clip~\eqref{eq:irls_weights} turns IRLS into an MM iteration on a \emph{smooth} surrogate of the objective: the $L_1$ term is effectively replaced by a function equal to $|w_i|$ for $|w_i| > \varepsilon_{\mathrm{thr}}$ and quadratic for $|w_i| \le \varepsilon_{\mathrm{thr}}$, a Huber-type smoothing. By Ostrowski's theorem of attraction~\cite{ortega1970iterative}, $\mathbf{w}^*$ is then a point of attraction with local linear rate $\rho\big(\nabla\mathcal{M}(\mathbf{w}^*)\big)$. A complete proof of the hypothesis of the theorem is reported in Appendix \ref{app:irls-local-conv-proof}.

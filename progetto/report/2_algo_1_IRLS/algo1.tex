...
The clip~\eqref{eq:irls_weights} turns IRLS into an MM iteration on a \emph{smooth} surrogate of the objective: the $L_1$ term is effectively replaced by a function equal to $|w_i|$ for $|w_i| > \varepsilon_{\mathrm{thr}}$ and quadratic for $|w_i| \le \varepsilon_{\mathrm{thr}}$, a Huber-type smoothing. Under full column rank of $\mathbf{X}$ this smoothed objective is strongly convex ($\mathbf{X}^T\mathbf{X} \succ 0$, Proposition~\ref{prop:irls_prop}), and the MM map $\mathbf{w}_{k+1} = \mathbf{Q}_k^{-1}\mathbf{b}$ is a contraction near its fixed point, so IRLS converges \emph{locally linearly} to the smoothed minimiser. The asymptotic factor is the contraction of that map, which here is close to $1$: the quadratic surrogate of $|w_i|$ is loose on the active components, where $|w_i|$ is locally affine (zero curvature) while the surrogate imposes curvature $\lambda_{\text{LASSO}}/|(w^*)_i|$, and that mismatch slows the contraction. Section~\ref{sec:convergence} measures the factor ($\approx 0.99$ on the convergence instance) and finds it essentially independent of $\varepsilon_{\mathrm{thr}}$. We do \emph{not} invoke the $\ell_q$ linear-rate result of~\cite[Theorem~5.3]{daubechies2008iterativelyreweightedsquaresminimization}: it assumes a null space property for \emph{underdetermined} sparse recovery, whereas our regime is overdetermined ($M > H$) and strongly convex. The smoothed minimiser differs from the LASSO optimum by $O(\varepsilon_{\mathrm{thr}})$ in objective (item~2 of the hypothesis verification below).

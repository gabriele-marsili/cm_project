...
The clip~\eqref{eq:irls_weights} turns IRLS into an MM iteration on a \emph{smooth} surrogate of the objective: the $L_1$ term is effectively replaced by a function equal to $|w_i|$ for $|w_i| > \varepsilon_{\mathrm{thr}}$ and quadratic for $|w_i| \le \varepsilon_{\mathrm{thr}}$, a Huber-type smoothing $f_\varepsilon$ that is strongly convex under full column rank of $\mathbf{X}$. Writing the iteration as the fixed-point map $\mathcal{M}(\mathbf{w}) = \mathbf{Q}(\mathbf{w})^{-1}\mathbf{b}$, with $\mathbf{Q}(\mathbf{w}) = \mathbf{X}^T\mathbf{X} + \lambda_{\text{LASSO}}\,\mathrm{diag}\!\big(1/\max(|w_i|,\varepsilon_{\mathrm{thr}})\big)$, the minimiser $\mathbf{w}^*$ is a fixed point at which $\mathcal{M}$ is $C^1$ with spectral radius $\rho\big(\nabla\mathcal{M}(\mathbf{w}^*)\big) < 1$. By Ostrowski's theorem of attraction~\cite{ortega1970iterative}, $\mathbf{w}^*$ is therefore a point of attraction with local linear rate $\rho\big(\nabla\mathcal{M}(\mathbf{w}^*)\big)$, close to $1$ because the surrogate over-curves the affine $L_1$ penalty on the active set. The Jacobian, the spectral-radius bound, and the verification that the theorem's hypotheses hold are derived in Appendix~\ref{app:irls-local-conv-proof}.

We do \emph{not} invoke the $\ell_q$ linear-rate result of~\cite[Theorem~5.3]{daubechies2008iterativelyreweightedsquaresminimization}: it assumes a null space property for \emph{underdetermined} sparse recovery, whereas our regime is overdetermined ($M > H$) and strongly convex. The smoothed minimiser differs from the LASSO optimum by $O(\varepsilon_{\mathrm{thr}})$ in objective (item~2 of the hypothesis verification below).

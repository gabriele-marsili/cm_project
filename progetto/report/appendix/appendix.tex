...
The safety threshold $\varepsilon$ introduced in the weights formula prevents division by zero. However, it also changes \ref{eq:obj_fun} into a smoothed one:
\begin{equation}
...
The update process of the IRLS algorithm defines an iteration map $\mathcal{M}(\mathbf{w})=\mathbf{Q}(\mathbf{w})^{-1}\mathbf{b}$.
Because the optimal solution $\mathbf{w}^*$ satisfies the minimum conditions of the smoothed objective, it is a fixed point of this map, meaning $\mathcal{M}(\mathbf{w}^*)=\mathbf{w}^*$.
...
Comparing their diagonal elements, we can observe a specific pattern. 
...
If we calculate the derivative $d\mathbf{Q}$ and multiply it by $\mathbf{w}^*$, the result perfectly matches the difference between these two matrices. We can formalize this relationship as:
\begin{equation}
...
\[
    \mathbf{B}-\mathbf{H}
    = \lambda_{\mathrm{LASSO}}\,\mathrm{diag}\!\left(\frac{1}{|w^*_i|}\text{ on }\mathcal{A},\ 0\text{ on }\mathcal{P}\right).
\]
...
The eigenvalues of our Jacobian matrix $\mathbf{I} - \mathbf{B}^{-1}\mathbf{H}$ are simply $1 - \mu_i$.

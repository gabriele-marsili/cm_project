...
via the subgradient inequality alone.
The key step — establishing that the constraint $\beta_i \leq \gamma_i$ still guarantees a per-step decrease of the form
\begin{equation}
...
\end{equation}
— does not follow from the subgradient inequality and is the content of \cite[Theorem~3.2]{dantonio2009deflected}.
Under this guarantee, the same telescoping argument as in Steps~2--3 (with $\norm{\mathbf{d}_i}_2$ in place of $\norm{\mathbf{g}_i}_2$) directly yields the $O(L^2/\varepsilon^2)$ bound. Since $\mathbf{d}_i$ is a convex combination of past subgradients, $\norm{\mathbf{d}_i}_2 \leq \norm{\mathbf{g}_i}_2 \leq L$, so the same constant $L$ applies.

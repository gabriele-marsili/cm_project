...
\end{equation}
At a component where $w_i \ne 0$ the subdifferential is a singleton and we have no choice but $s_i = \operatorname{sign}(w_i)$; at $w_i = 0$, the subdifferential is the whole interval $[-1, +1]$ and we pick the reference value $s_i = 0$. A more sophisticated way to define the value of $s_i$ could be made in order to minimize the Euclidean norm of the subgradient $g$. The decision doesn't have much impact because the probability of having a $w_i = 0$ in the algorithm, which uses floating-point precision, is relatively low.
...
(Note: this derivation is for the plain subgradient case $\gamma_i = 1$, where $\mathbf{d}_i = \mathbf{g}_i$ and $\norm{\mathbf{d}_i}^2 = \norm{\mathbf{g}_i}^2$; the deflected case is treated below.)

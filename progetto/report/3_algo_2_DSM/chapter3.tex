...
&= \gamma^2\bigl(\norm{\mathbf{g}_i}^2
                ...
                + \norm{\mathbf{d}_{i-1}}^2\bigr)
  + 2\gamma\bigl(\inner{\mathbf{g}_i}{\mathbf{d}_{i-1}}
                - \norm{\mathbf{d}_{i-1}}^2\bigr)
  + \norm{\mathbf{d}_{i-1}}^2. \label{eq:gamma_expand}
\end{align}
Recognising the first factor as $\norm{\mathbf{g}_i - \mathbf{d}_{i-1}}^2$:
\begin{equation}
h(\gamma)
= \norm{\mathbf{g}_i - \mathbf{d}_{i-1}}^2\,\gamma^2
  + 2\bigl(\inner{\mathbf{g}_i}{\mathbf{d}_{i-1}} - \norm{\mathbf{d}_{i-1}}^2\bigr)\gamma
  + \norm{\mathbf{d}_{i-1}}^2.
\label{eq:gamma_parabola}
\end{equation}

\textbf{Case $\mathbf{g}_i = \mathbf{d}_{i-1}$:} the leading coefficient $\norm{\mathbf{g}_i - \mathbf{d}_{i-1}}^2 = 0$, so $h(\gamma) = \norm{\mathbf{d}_{i-1}}^2$ is constant on $[0,1]$; every $\gamma$ attains the minimum and we set $\gamma_i = 1$ by convention.
...
\[
h'(\gamma)
= 2\norm{\mathbf{g}_i - \mathbf{d}_{i-1}}^2\,\gamma
  + 2\bigl(\inner{\mathbf{g}_i}{\mathbf{d}_{i-1}} - \norm{\mathbf{d}_{i-1}}^2\bigr) = 0
\implies
\gamma^* = \frac{\norm{\mathbf{d}_{i-1}}^2 - \inner{\mathbf{g}_i}{\mathbf{d}_{i-1}}}
               {\norm{\mathbf{g}_i - \mathbf{d}_{i-1}}^2},
\]
...
The denominator $\norm{\mathbf{g}_i - \mathbf{d}_{i-1}}^2$ measures the squared distance between the current subgradient and the previous direction: the larger it is, the more $\gamma^*$ shrinks and the more the deflected direction borrows from memory $\mathbf{d}_{i-1}$. The numerator $\norm{\mathbf{d}_{i-1}}^2 - \inner{\mathbf{g}_i}{\mathbf{d}_{i-1}}$ captures alignment: it is small (and $\gamma^*$ small) when $\mathbf{g}_i$ and $\mathbf{d}_{i-1}$ are well aligned, and large when they oppose each other. In the zig-zag scenario $\mathbf{g}_i \approx -\mathbf{d}_{i-1}$ with comparable magnitudes $\norm{\mathbf{g}_i} \approx \norm{\mathbf{d}_{i-1}}$, the numerator is approximately $\norm{\mathbf{d}_{i-1}}^2 + \norm{\mathbf{d}_{i-1}}^2 = 2\norm{\mathbf{d}_{i-1}}^2$ while the denominator is approximately $\norm{2\mathbf{d}_{i-1}}^2 = 4\norm{\mathbf{d}_{i-1}}^2$, giving $\gamma^* \approx 1/2$: $\mathbf{d}_i$ splits evenly between the two conflicting directions, halving the oscillation.
...
\label{eq:proof_rate}
\bar{f}_k - f^*
\leq \frac{L\,\norm{\mathbf{w}_0 - \mathbf{w}^*}}{\sqrt{k}}.
\end{equation}
To achieve $\bar{f}_k - f^* \leq \varepsilon$ it suffices to run $k \geq L^2\norm{\mathbf{w}_0 - \mathbf{w}^*}^2/\varepsilon^2$ iterations, establishing the $O(L^2/\varepsilon^2)$ bound.

\medskip
\noindent\textbf{Extension to deflected directions ($\gamma_i < 1$, $\beta_i \leq \gamma_i$).}
When the update direction $\mathbf{d}_i = \gamma_i\,\mathbf{g}_i + (1-\gamma_i)\,\mathbf{d}_{i-1}$ is not a subgradient,
the inner product $\inner{\mathbf{d}_i}{\mathbf{w}_i - \mathbf{w}^*}$ cannot be directly bounded below by $f(\mathbf{w}_i) - f^*$
via the subgradient inequality.
The complete convergence analysis for the stepsize-restricted deflected case
$\alpha_i = \beta_i(f(\mathbf{w}_i) - f^*)/\norm{\mathbf{d}_i}^2$ with $\beta_i \leq \gamma_i$
is carried out in \cite[Theorem~3.2]{dantonio2009deflected}.
The key mechanism is that the constraint $\beta_i \leq \gamma_i$ limits the step length relative to the weight placed on the subgradient component, ensuring a per-step decrease analogous to \eqref{eq:proof_perstep} with $\norm{\mathbf{d}_i}$ in place of $\norm{\mathbf{g}_i}$. The same telescoping argument then yields the $O(L^2/\varepsilon^2)$ bound, with $L$ now a uniform bound on $\norm{\mathbf{g}_i}$ (which also bounds $\norm{\mathbf{d}_i}$ since $\mathbf{d}_i$ is a convex combination of past and present subgradients).
...
From \eqref{eq:proof_rate}, $\bar{f}_k - f^* \leq L\norm{\mathbf{w}_0 - \mathbf{w}^*}/\sqrt{k} \to 0$ as $k \to \infty$. Since $\bar{f}_k \geq f^*$ by definition, we conclude $\bar{f}_k \to f^*$.
\end{proof}

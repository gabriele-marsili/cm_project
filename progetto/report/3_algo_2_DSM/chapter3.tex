...
Algorithm~\ref{alg:dsm} implements the SGPTL rule of slide~14 (Lessons~Optimization) and Lemma~3.8 of~\cite{dantonio2009deflected}: lines~12--13 are the stepsize-restricted Polyak step, lines~16--21 the three-branch update of $(f_{\mathrm{ref}}, \delta, r)$. The only deviation from the slide is the clip on $\gamma_i$ at line~9 onto $[\gamma_{\min}, 1]$, which realises condition~(3.5) of~\cite{dantonio2009deflected} with $\beta^* = \gamma_{\min}$. Setting $\gamma_0 = 1$ realises the convention $\mathbf{d}_{-1} = \mathbf{0}$ in~\eqref{eq:deflected_dir}, since~\eqref{eq:gamma_star} is $0/0$ at $i=0$.
...
\paragraph{$\beta = 1$.}
\cite[(3.1)]{dantonio2009deflected} admits $\beta_k \in (0, 1]$; the value $\beta = 1$ minimises the one-step distance bound \cite{polyak-stepsize} (slide~12), and the deflected variant of slide~15 inherits the property. We fix $\beta = 1$ so that $\beta_i = \min(\beta, \gamma_i)$ is driven by $\gamma_i$.

...
Without a floor, the closed-form minimiser of~\eqref{eq:gamma_opt} is projected onto $0$ whenever consecutive subgradients align (the parabola coefficients in~\eqref{eq:gamma_parabola} satisfy $\norm{\mathbf{d}_{i-1}}_2^2 - \inner{\mathbf{g}_i}{\mathbf{d}_{i-1}} \to 0^+$ near stationarity). Then $\gamma_i = \beta_i = \alpha_i = 0$ and the iterate freezes. Projecting onto $[\gamma_{\min}, 1]$ with $\gamma_{\min} = 0.05$ realises condition~(3.5) of~\cite{dantonio2009deflected} with $\beta^* = \gamma_{\min}$. A sweep $\gamma_{\min} \in \{0.05, 0.1, 0.2, 0.5\}$ on the convergence instance gives record gaps in $[10^{-7}, 10^{-5}]$, with $\gamma_{\min} = 0.05$ marginally best. The freeze in the unfloored case is not a $(\delta_0, \rho)$ tuning artifact: a $25$-point sweep with $\gamma_{\min} = 0$ produces only one configuration with gap below $10^{-3}$, and that gap still stalls two orders of magnitude above the floored variant.

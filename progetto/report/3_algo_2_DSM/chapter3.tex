...
The literal stepsize-restricted reading sets $\beta_i = \min(\beta,\gamma_i)$ with a fixed reference value $\beta \in (0,1]$. With $\beta = 1$ the clipping is active whenever the greedy deflection pushes $\gamma_i$ below one. We \emph{do not} adopt this clip in the implementation: on ELM~LASSO the greedy choice~\eqref{eq:gamma_star} drives $\gamma_i\to 0$ within a few tens of iterations on every problem instance we tested, the clip then drives $\beta_i$ and $\alpha_i$ to zero, the iterate freezes, and the run stalls at a record gap roughly two orders of magnitude looser than what the same budget reaches with $\beta$ held fixed (Section~\ref{sec:setup} of Chapter~\ref{chap:results} reports both runs side by side). We therefore set $\beta = 1$ and use it without clipping; the Polyak step in eq.~\eqref{eq:sr_step} reads $\alpha_i = (f(\mathbf{w}_i) - f^*)/\norm{\mathbf{d}_i}_2^2$ for $\beta = 1$, the standard Polyak step on the deflected direction. The trade-off is that the per-step iterate-distance bound from Theorem~3.2 of~\cite{dantonio2009deflected} no longer applies verbatim to iterations where $\gamma_i < 1$ (so $\mathbf{d}_i$ is not a subgradient); the convergence guarantee is recovered through the iteration-count fallback (Section~\ref{sec:setup}), which periodically resets $\mathbf{d}_{i-1}\gets\mathbf{0}$ and forces $\gamma=1$ at the next iterate. The proof of Theorem~\ref{thm:dsm_convergence} then applies to the resulting $\gamma=1$ subsequence, which is sufficient for $\bar{f}^{i}\to f^*$.
...
\gamma^* = \frac{\norm{\mathbf{d}_{i-1}}_2^2 - \inner{\mathbf{g}_i}{\mathbf{d}_{i-1}}}{\norm{\mathbf{g}_i - \mathbf{d}_{i-1}}_2^2}, \quad \gamma_i = \min(1,\max(0,\gamma^*)).
\end{equation}
...
The constrained minimum over $[0,1]$ is $\gamma_i = \min(1,\max(0,\gamma^*))$: since the parabola opens upward, if $\gamma^* < 0$ then $h$ is increasing on $[0,1]$ and the minimum is attained at $\gamma = 0$; if $\gamma^* > 1$ then $h$ is decreasing on $[0,1]$ and the minimum is at $\gamma = 1$; if $\gamma^* \in [0,1]$ the interior solution applies.
...
\EndIf
\State $\beta_i \gets \beta$ \hfill\emph{(see \S\ref{sec:dsm_convergence_conditions}: $\beta=1$ fixed, no clip)}
...
\end{equation}
does not follow from the subgradient inequality directly. Under the literal
stepsize-restricted condition $\beta_i \le \gamma_i$ used in
\cite[Theorem~3.2]{dantonio2009deflected}, the bound is recovered for every
iteration. We do not adopt that clip in the implementation
(Section~\ref{sec:dsm_convergence_conditions} explains why), so for the
intermediate iterations with $\gamma_i < 1$ the bound~\eqref{eq:proof_perstep_deflected}
is no longer a theorem; the convergence guarantee instead operates
\emph{along the subsequence of resets} $\{i_k\}$ at which the iteration-count
fallback (Chapter~\ref{chap:results}, Section~\ref{sec:setup}) sets
$\mathbf{d}_{i_k - 1} \gets \mathbf{0}$ and the next step uses
$\gamma_{i_k} = 1$, hence $\mathbf{d}_{i_k} = \mathbf{g}_{i_k}$. The plain
Polyak step on a true subgradient applies at every such $i_k$, so the
telescoping argument of Steps~2--3 (now summed over the
$\{i_k\}$-subsequence) yields
$\bar{f}_{i_k} - f^* \le L\,\norm{\mathbf{w}_0 - \mathbf{w}^*}_2 / \sqrt{|\{j : j \le i_k\}|}$ \,
which is sufficient to conclude $\bar{f}^{i} \to f^*$ as $i \to \infty$.
The $O(L^2/\varepsilon^2)$ rate is preserved as long as the fallback fires
$\Theta(i)$ times in any $i$-iteration window, which our
$R_{\mathrm{iter}} = \max(i_{\max}/100,\,50)$ guarantees by construction.

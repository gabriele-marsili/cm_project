...
Condition~(3.5) of~\cite{dantonio2009deflected} requires $\gamma_i \ge \beta^* > 0$ \emph{uniformly} in $i$, not just pointwise. The closed-form greedy~\eqref{eq:gamma_opt} gives $\gamma_i \in (0,1]$ at each step but does not bound the sequence away from zero: when consecutive subgradients align near stationarity, the numerator $\norm{\mathbf{d}_{i-1}}_2^2 - \inner{\mathbf{g}_i}{\mathbf{d}_{i-1}}$ in~\eqref{eq:gamma_parabola} tends to $0^+$ and $\gamma_i$ decays, dragging $\beta_i = \min(1, \gamma_i)$ and the Polyak step to zero --- the iterate freezes. Projecting onto $[\gamma_{\min}, 1]$ with $\gamma_{\min} = 0.05$ supplies the uniform bound $\beta^* = \gamma_{\min}$. A sweep $\gamma_{\min} \in \{0.05, 0.1, 0.2, 0.5\}$ on the convergence instance gives record gaps in $[10^{-7}, 10^{-5}]$, with $\gamma_{\min} = 0.05$ marginally best. The freeze in the unfloored case is not a $(\delta_0, \rho)$ tuning artifact: a $25$-point sweep with $\gamma_{\min} = 0$ produces only one configuration with gap below $10^{-3}$, and that gap still stalls two orders of magnitude above the floored variant.

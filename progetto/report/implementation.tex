...
A preconditioned CG loop with diagonal (Jacobi) preconditioner
$\mathbf{M}^{-1} = \operatorname{diag}(\mathbf{Q}_k)^{-1}$, well defined since
$\mathbf{Q}_k$ has strictly positive diagonal.
The iteration terminates when the relative residual
$\|\mathbf{r}_k\| / \|\mathbf{b}\| < \texttt{tol}$ (default $10^{-10}$).
In exact arithmetic it converges in at most $H$ steps.
...
\begin{enumerate}[nosep]
    \item Draw $\mathbf{X}_{\mathrm{raw}} \in \mathbb{R}^{M \times n}$ with i.i.d.\
    $\mathcal{N}(0,1)$ entries, then normalise each column to unit $\ell_2$ norm.
    ...
    with \texttt{alpha} $= \lambda/M$ to machine precision. Scikit-learn minimises
    $(1/(2M))\|\mathbf{Xw} - \mathbf{y}\|_2^2 + \alpha\|\mathbf{w}\|_1$, which shares
    its minimiser with our $f(\mathbf{w}) = (1/2)\|\mathbf{Xw} - \mathbf{y}\|_2^2 +
    \lambda\|\mathbf{w}\|_1$ exactly when $\alpha = \lambda/M$.
\end{enumerate}
The companion \texttt{make\_elm\_problem} follows the same recipe except that
the design $\mathbf{X}$ is replaced by $\mathbf{X}_{\mathrm{hidden}} =
\sigma(\mathbf{X}_{\mathrm{raw}}\mathbf{W}_1^T)$ with a random ELM hidden layer,
and no column renormalisation is applied: bounded activations already keep the
columns in a fixed range.
